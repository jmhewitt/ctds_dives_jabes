#
# discrete Cramer von Mises gof test formatting
#

cvm.gof = function(df, var, verbose = TRUE) {
...
cvm.test = function (x, y, type = c("W2", "U2", "A2"), simulate.p.value = FALSE, 
                             B = 2000, tol = 1e-08) 
{
  ...
  cvm.stat.disc <- function(x, y, type = c("W2", "U2", "A2")) {
    ...
    M <- switch(type, W2 = E, 
                U2 = (diag(1, nrow(E)) - 
                        E %*% One %*% t(One)) %*% E %*% 
                  (diag(1, nrow(E)) - One %*% t(One) %*% E), A2 = E %*% K)
    lambda <- eigen(M %*% Sy)$values
    STAT <- switch(type, W2 = sum(Z^2 * t)/N, 
                   U2 = sum((Z - Zbar)^2 * t)/N, 
  ...
  if (is.stepfun(y)) {
    ...
    tempout <- Mod(cvm.stat.disc(x, y, type = type))
    STAT <- tempout[1]
